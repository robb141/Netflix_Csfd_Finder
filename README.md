...
- Seen/unseen matching between TMDB and a user's rated csfd titles is on a normalized key (case, accents, dashes, curly apostrophes and whitespace folded away) plus a year check when both sides know the year, not an exact string match - so this is fairly tolerant of translation/transliteration differences, but not infallible.
- Not every unseen title ends up with a `percentage`. csfd.cz's own search is a loose full-text match rather than a title search - it also matches on descriptions, episode subtitles, cast/crew names, etc. - so a search result sharing the right title text or the right year is not, on its own, reliable proof it's the right film. `find_csfd_film_url` in `main.py` only accepts a result it's confident in (see its docstring for the exact rules) and otherwise leaves `percentage` empty rather than reporting a wrong film's rating. In practice this means a lot of obscure or Netflix-exclusive titles that simply aren't in csfd's database end up with no percentage at all - that's expected, not a bug.
